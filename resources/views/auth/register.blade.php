...
.field input,.field select{width:100%;padding:14px 16px;border-radius:14px;border:1px solid var(--line);font-size:14px;background:#f9fbff;}
...
<div class="wrap">
  ...
  <section class="right">
    <div class="card">
      <h1>Demande de crédit sociétaire</h1>
      <p>Créez un compte et soumettez dès maintenant une demande de crédit. Votre dossier sera enregistré avec le statut « Reçue ».</p>
      ...
      <form method="POST" action="{{ route('register.submit') }}">
        @csrf
        <div class="field"><label>Agence</label><select name="agence_id" required>
        ...
        <div class="field"><label>Nom</label><input type="text" name="nom" value="{{ old('nom') }}" required></div>
        <div class="field"><label>Prénom</label><input type="text" name="prenom" value="{{ old('prenom') }}" required></div>
        <div class="field"><label>Téléphone</label><input type="text" name="telephone" value="{{ old('telephone') }}" required></div>
        <div class="field"><label>Email (facultatif)</label><input type="email" name="email" value="{{ old('email') }}"></div>

        <div class="field"><label>Type de crédit</label><select name="type" id="type" required onchange="toggleTypeFields()">
            <option value="ordinaire" {{ old('type') === 'ordinaire' ? 'selected' : '' }}>Crédit ordinaire</option>
            <option value="partenariat" {{ old('type') === 'partenariat' ? 'selected' : '' }}>Crédit de partenariat</option>
          </select></div>

        <div class="field" id="sousTypeBlock" style="display:none;">
          <label>Sous-type (crédit ordinaire)</label>
          <select name="sous_type">
            <option value="">— Sélectionnez —</option>
            @foreach(['investissement','exploitation','financement_marche','salaire','consommation','agricole','energies_vertes','activites_culturelles'] as $st)
        ...
        <div class="grid g2">
          <div class="field"><label>Montant sollicité (F CFA)</label><input type="number" step="0.01" name="montant" value="{{ old('montant') }}" required></div>
          <div class="field"><label>Durée (mois)</label><input type="number" name="duree_mois" value="{{ old('duree_mois', 12) }}" required></div>
        </div>
        ...
        <div class="field"><label>Confirmation du mot de passe</label><input type="password" name="password_confirmation" required></div>
        <button class="btn" type="submit">Créer mon compte et déposer ma demande</button>
      </form>

      <script>
        function toggleTypeFields() {
          const type = document.getElementById('type').value;
          document.getElementById('sousTypeBlock').style.display = type === 'ordinaire' ? 'block' : 'none';
          document.getElementById('partenaireBlock').style.display = type === 'partenariat' ? 'block' : 'none';
        }
